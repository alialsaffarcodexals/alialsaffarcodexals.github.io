 ...
 * Transactions table UI: filtering, range sliders, pagination, and rendering.
 */
import { debounce } from '../../utils/debounce.js';
import { formatXP, formatXPStat } from '../../utils/format.js';
import { projectFromPath } from '../../utils/path.js';

// initTransactionsTable: Initializes transactions table filters, listeners, and first render.
export function initTransactionsTable(data) {
  ...
  if (allTransactions.length && xpRange && dateRange && window.noUiSlider) {
    ...
    // updateLabels: Updates slider label text from the current noUiSlider values.
    const updateLabels = () => {
  ...
  // filterTransactions: Applies text, XP, and date filters to the full transactions dataset.
  ...
  // renderTransactions: Renders the visible transactions page and toggles load-more control.
  ...
  // resetSliders: Resets slider controls to their full min/max range.
  const resetSliders = () => {
