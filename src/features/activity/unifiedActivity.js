/**
 * unifiedActivity.js
 * Builds and renders combined recent project and audit activity lists.
 */
import { formatXP, formatXPStat, formatGrade } from '../../utils/format.js';
import { projectFromPath } from '../../utils/path.js';

// renderUnifiedActivity: Renders the unified activity widget sections.
export function renderUnifiedActivity(data) {
  renderRecentProjects(data);
  renderRecentAudits(data);
}

// renderRecentProjects: Builds recent project updates from transactions/results/progress records.
function renderRecentProjects(data) {
  ...
  // addEntry: Adds or updates a project entry while keeping the newest timestamp.
...
// renderRecentAudits: Builds and renders recent audit activity (done vs received).
    ...
    auditsDoneList.innerHTML = rows.length
      ? rows
          .map((t) => {
            ...
            // Use precise XP format here, so small values do not become 0 kB.
