...
export function renderProfilePage(data, objectMap = {}) {
  ...
  // Normalize: API may return amount as number or numeric string
  const validTransactions = transactions
    .map((t) => ({ ...t, amount: Number(t?.amount) }))
    .filter((t) => t && t.createdAt && Number.isFinite(t.amount));

  // Total XP: platform totals usually exclude exercise & exam XP. Object types differ between accounts.
  // Strategy:
  // - If we have transaction.objectId + objectMap, exclude object types containing "exercise" or "exam".
  // - Otherwise, fall back to summing all XP transactions.
  // We'll compute this once and reuse for skills % as well
  let totalXP = null;
  ...
  if (totalXPEl) {
    ...
    const filtered = canUseObjectType
      ? validTransactions.filter((t) => {
          ...
          // keep unknown types (""), but drop obvious non-project XP
          if (!type) return true;
    ...
    // If we have a schema-provided `user.xps(...)` total list (best-effort query), prefer it for Total XP.
    // This can match the platform total more closely for some accounts.
  ...
  if (passFailChart || passFailRatioEl) {
    // Prefer pre-filtered project results from the API query
    ...
    // Fallback to raw results if needed
  ...
  // Audits Done vs Received (uses transactions: up/down)
  if (auditsChart || auditsRatioEl) {
    ...
    // Use XP (amount) for the donut ratio, not raw counts.
    // Note: the UI already formats these values as B/kB/MB elsewhere; here we keep raw numbers for ratio.
    ...
    // Update chart center + caption to reflect audits
    const svg = auditsChart?.querySelector('svg');
    const texts = svg ? Array.from(svg.querySelectorAll('text')) : [];

    // Chart creates:
    // [0] big number, [1] label ("Total"), [2] caption ("Pass: ... | Fail: ...")
  ...
  if (xpByProjectChart) {
    // Prefer topProjects from the API query (best-effort filtering by path to exclude exams/exercises).
    ...
    if (topProjects.length) {
      ...
      // Fallback: aggregate from all XP transactions.
      const xpByProject = {};
